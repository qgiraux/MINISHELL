...
/*   Updated: 2024/03/11 11:52:36 by qgiraux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ms_exp_filename_match_madd(\
...
static int	ms_exp_filename_match_madd_cwd(\
	char *pattern, t_dlist **match, void *data)
{
	t_dlist	*file;
	char	*name;
	int		file_dot;
	...
	while (file)
	{
		...
		{
			if (ms_exp_filename_match_madd(name, pattern, match))
				return (ms_e(__FILE__, __LINE__, 0), 1);
		}
		file = file->next;
	}
...
int	ms_exp_filename(t_dlist **escpd_token, t_dlist **next, void *data)
{
	...
	{
		new = NULL;
		if (ms_exp_filename_match_madd_cwd(curr->content, &new, NULL))
			return (ms_e(__FILE__, __LINE__, 0), \
				ms_dlstclear(&new), 1); //dlstclear non rec
		if (NULL != new)
		{
			*next = ms_dlstreplace(escpd_token, &curr, new); // FIX ME <*cat
			return (MS_SUCCESS); //
		}
	}
	*next = curr->next;
...
// 		ms_exp_filename_match_madd(filename, pattern, &match);
// 	}
// 	if (2 == ac)
// 	{
// 		pattern = ft_strdup(av[1]);
// 		printf("pattern:%s\n",  pattern);
// 		ms_exp_filename_match_madd_cwd(pattern, &match, NULL);
// 		while (match)
// 		{
// 			printf("file:%s\n", (char *)(match->content));
// 			match = match->next;
