...
/*   Updated: 2024/02/23 17:03:16 by qgiraux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		parser_error(t_dlist *list, const char **data)
{
	...
	while (list != NULL)
	{
		...
		if (1 == ms_dlist_istype_pipe_and_or(list) || list->type == MS_TOKEN_OPEN)
		{

			if (1 == ms_dlist_istype_pipe_and_or(list->next) || list->next->type == MS_TOKEN_CLOSE)
			{
