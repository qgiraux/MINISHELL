...
/*   Updated: 2024/03/25 14:12:42 by jerperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*_path_mget_row(char *line, size_t *line_index, char sep)
{
	size_t	len_row;
	size_t	i;
	char	*row;
...
static char	*_path_mjoin(const char *path, const char *bin_name)
{
	...
	char **bin_path, const char *bin_name, const char **env)
{
	...
	{
		path = _path_mget_row(line, &line_index, ENV_PATH_SEP);
		if (NULL == path)
			return (free(line), ms_e(__FILE__, __LINE__, 0), 1);
		*bin_path = _path_mjoin(path, bin_name);
		free(path);
		if (NULL == *bin_path)
			return (free(line), ms_e(__FILE__, __LINE__, 0), 1);
		if (0 != errno)
			return (free(line), ms_e(__FILE__, __LINE__, 1), 1);
