...
/*   Updated: 2024/03/25 14:28:23 by jerperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	_curr_unescape(t_dlist **head)
{
	char	*str;
	t_dlist	*curr;
...
static int	_curr_expand(t_dlist **head, char **env, void *data)
{
	char	*str;
	t_dlist	*curr;
	t_dlist	**ptr_curr;
...
int	ms_exp_str(const char *str, t_dlist **exp, void *data)
{
	...
	*exp = ms_dlstnew(dup, MS_TOKEN_WORD);
	if (NULL == *exp)
		return (ms_perror(MS_E), 1);
	if (_curr_expand(exp, env, data))
		return (ms_e(__FILE__, __LINE__, 0), 1);
	if (_curr_unescape(exp))
		return (ms_e(__FILE__, __LINE__, 0), 1);
	return (MS_SUCCESS);
}
...
int	ms_exp(t_dlist **head, void *data)
{
	...
	env = ms_data_env_get(data);
	if (_curr_expand(head, env, data))
		return (ms_e(__FILE__, __LINE__, 0), 1);
	if (_curr_unescape(head))
		return (ms_e(__FILE__, __LINE__, 0), 1);
	return (MS_SUCCESS);
}
