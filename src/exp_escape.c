...
/*   Updated: 2024/03/28 14:54:08 by jerperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "exp_utils.h"

static char	*_replace(char *str, char *new, char *list)
{
	size_t	i_str;
	size_t	i_new;
...
static void	_removal_write(char *str, char *new, char *list)
{
	size_t	i_str;
	size_t	i_new;
...
char	*ms_exp_escape_dup(char *str, char *to_escape)
{
	...
	dup = ms_exp_escape(str, to_escape);
	if (NULL == dup)
...
char	*ms_exp_escape(char *str, char *to_escape)
{
	...
	if (NULL == str_esc)
		return (ms_perror(MS_E), NULL);
	return (_replace(str, str_esc, to_escape));
}
